...
<?php if ( ! empty( $youtube_url ) ): ?>
    <div class="sc-video">
        <div class="sc-video__bg-mb">
            <iframe width="100%" height="560" src="<?php echo esc_url( $youtube_url ) ?>?rel=0&autoplay=1&loop=1"
            ...
            <div class="row intro-content">
                <div class="col-md-12">
					...
					<?php if ( ! empty( $special_site_url ) ): ?>
                        <p><?php echo translate_text_language( '1x' ) ?>: <a class="has-decoration"
                                                                             href="<?php echo $special_site_url ?>">DESIGNARTTOKYO 2018 </a></p>
					<?php endif; ?>

					<?php if ( ! empty( $exhibitor_list_url ) ): ?>
                        <p><?php echo translate_text_language( '2x' ) ?>: <a class="has-decoration"
                                                                             href="<?php echo esc_url( $exhibitor_list_url ) ?>">DESIGNARTTOKYO 2018 <?php echo translate_text_language( '2x' ) ?></a></p>
					<?php endif; ?>

					<?php if ( ! empty( $pdf1_url ) && ! empty( $pdf2_url ) ): ?>
                        <p class="download"><?php echo translate_text_language( '3x' ) ?>
                            (PDF): <?php echo translate_text_language( '4x' ) ?>
                            <a href="<?php echo esc_url( $pdf1_url ) ?>"><span
                                        class="pdf-icon"></span></a>/ <?php echo translate_text_language( '5x' ) ?>
                    ...
                    <div class="row img-block">
                        ...
                        <div class="col-md-12 img-list">
							<?php foreach ( $gallery as $img ): ?>
								...
								$data_info_image = '';
								if ( ! empty( $img ) ) {
									$product_name      = get_field( $prefix_varible . 'product_name', $img );
									$exhibit_place     = get_field( $prefix_varible . 'exhibit_place', $img );
									$photographer_name = get_field( $prefix_varible . 'photographer_name', $img );
									$data_info_image   = array(
										'product_name'  => ! empty( $product_name ) ? $product_name : '',
										'exhibit_place' => ! empty( $exhibit_place ) ? $exhibit_place : '',
										'photo_name'    => ! empty( $photographer_name ) ? $photographer_name : '',
									);
									$thumb_url         = wp_get_attachment_image_src( $img, 'designart-thumb', true );
									if ( ! empty( $thumb_url ) ) {
								...
								if ( ! empty( $img_url ) ) :
                                    ...
                                    <div class="img-item">
                                        <div class="fancybox"
                                             data-caption="<?php echo urlencode( json_encode( $data_info_image ) ); ?>"
                                             data-fancybox="<?php echo esc_attr( strtolower( $title ) ) ?>"
                                             href="<?php echo esc_url( $img_url ) ?>"><img
                                                    src="<?php echo esc_url( $img_thumb ) ?>"
                        ...
                        <div class="col-md-12 img-list">
							<?php foreach ( $gallery as $img ): ?>
								...
								if ( ! empty( $img_url ) ) :
                                    ...
                                    <div class="img-item">
                                        <div class="fancybox"
                                             data-fancybox="m-<?php echo esc_attr( strtolower( $title ) ) ?>"
                                             href="<?php echo esc_url( $img_url ) ?>"
                                             title="<?php echo esc_attr( strtolower( $title ) ) ?>"><img
                                                    src="<?php echo esc_url( $img_thumb ) ?>"
                                                    alt="" class="img-responsive"></div>
                                    </div>
								<?php endif; ?>
    ...
    <div class="landing-st contact-st report">
        <div class="container">
            ...
            <div class="text-center group-btn">
                <a href="mailto:info@designart.jp" class="btn btn-bg yellow ">Contact about DESIGNART <i
                            class="icons fa fa-angle-right"></i></a>
                <a href="mailto:press@designart.jp" class="btn btn-bg yellow ">Contact about Press <i
                            class="icons fa fa-angle-right"></i></a>
            </div>
			<?php get_html_share(); ?>
        </div>
...
<?php
wp_enqueue_style( 'designart-report-css', URL_STATICS . '/css/report.css' );
wp_enqueue_script( 'designart-report-js', URL_STATICS . '/js/report.js', array( 'jquery', 'fancybox' ), false );
